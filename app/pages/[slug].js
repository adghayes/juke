...
export default function ArtistPage({ ownPage }){
  ...
  const { data, error, loading: artistLoading } = useSWR(artistKey, key => {
  ...
  return (
    <div className="static bg-gradient-to-tl from-green-200 to-purple-300 min-h-screen w-screen">
      <div className="lg:min-w-min max-w-screen-lg xl:max-w-screen-xl mx-auto min-h-screen bg-white flex flex-col sm:flex-row p-4">
        <aside className="pr-4 border-r flex flex-col items-stretch divide-y">
          <div className="py-4 flex flex-col items-center">
            ...
            />
            <p className="text-xl font-bold text-center py-2">{ artist && artist.display_name}</p>
            <p className="text-center">{ artist && artist.bio }</p>
          </div>
          <ul className="text-3xl divide-y text-left">
            { tabs.map(tab => {
              return (
                <li key={tab.id}>
                  <a 
                    href={`#${tab.id}`}
                    className={`block py-2 ${tab.id === openTab ? 'text-pink-600 font-bold' : ''}`}
